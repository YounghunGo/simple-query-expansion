...
def scroll_over_all_docs():
    for hit in helpers.scan(es, index='paper1'):
...
def loop_over_hashes_and_remove_duplicates():
    ...
    for hashval, array_of_ids in dict_of_duplicate_docs.items():
      if len(array_of_ids) > 1:
        ...
        matching_docs = es.mget(index="paper1", doc_type="json", body={"ids": array_of_ids})
        #print(matching_docs)
        for doc in matching_docs['docs']:
            ...
            #print("doc=%s\n" % doc)
            #print(doc['_id'])
            es.delete(index="paper1", doc_type="json", id=doc['_id'])
